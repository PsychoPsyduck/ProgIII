...
class userController implements IApiControler
{
  ...
  public function CargarUno($request, $response, $args)
  {
    ...
    if (
      ...
      && array_key_exists("fotoDos", $arrayArchivos) && !User::where('legajo', '=', $arrayDeParametros["legajo"])->exists()) {
        ...
        $user->clave = crypt($arrayDeParametros["clave"], "st"); 
        //Foto Uno
        ...
        $newResponse = "Carga exitosa".$response->withJson($userAMostrar, 200);
    } 
    else {
  ...
  public function IniciarSesion($request, $response)
  {
    ...
    $claveValida = strcasecmp($user[0]["clave"],crypt($arrayDeParametros["clave"], "st"));
    $si = 1;
    if($user[0]["clave"] == crypt($arrayDeParametros["clave"], "st"))
    {
      $si = 0;
    }
    if (count($user) == 1 && $si == 0  && $emailValido == 0) {
      unset($user[0]["clave"]);
